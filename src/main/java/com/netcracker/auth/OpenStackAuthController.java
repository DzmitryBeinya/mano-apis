...
public class OpenStackAuthController {
    ...
    public ResponseEntity<String> getTokens() throws IOException {
        ...
        File file = new ClassPathResource("responses/openstack-auth-get.json").getFile();
    ...
    public ResponseEntity<String> getToken() throws IOException {
        ...
        File file = new ClassPathResource("responses/openstack-auth-post.json").getFile();
        return new ResponseEntity<>(new String(Files.readAllBytes(Paths.get(file.getPath()))), httpHeaders, HttpStatus.OK);
    }
}
